...
public class InsertPersonCommand extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			int age = Integer.parseInt(request.getParameter("age"));  // NumFormatException 발생 가능
			...
			// 나이 : 정수 이외의 값이 입력됨
			response.setStatus(3001);  // 에러 코드 값을 작성, xhr.status 에러코드 값은 임의로 정한다
			...
			response.getWriter().println("입력 데이터의 확인하세요.");
		}
